...
- **多根连记**：长度框每行填一根，可一次整批录入；同一长度多根用 `85×5` 一次展开（5根各85），自动逐根记明细并汇总
- **默认长度单位**：设置里可选 毫米/公分/米，录入按所选单位换算，存储一律毫米
- **明细可改**：台账里每根的长度与备注都能修改，保存后汇总即时重算
...
- 明细台账：每个规格×颜色的逐条记录，可查看时间、备注，单根可改长度/备注、删除
